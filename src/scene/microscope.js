...
const PILLAR_FRONT = -6; // front face of the vertical pillar

const MAT = {
  cream: new THREE.MeshStandardMaterial({ color: 0xe7e0cf, roughness: 0.5, metalness: 0.05 }),
  creamDark: new THREE.MeshStandardMaterial({ color: 0xd6cdb9, roughness: 0.6, metalness: 0.05 }),
...
export function buildMicroscope() {
  ...
  base.add(mesh(box(25, 3.4, 33, 0.7), MAT.cream, { y: 1.7, z: -0.5 }));
  base.add(mesh(box(19, 2.2, 13, 0.6), MAT.cream, { y: 4.3, z: -9.5 }));                   // rear riser under the pillar
  base.add(mesh(box(15, 0.7, 13.5, 0.2), MAT.blackMatte, { y: 3.7, z: AXIS_Z + 0.5 }));     // black plateau
  // brightness control wheel, front-left, half sunk into the base edge
  const wheel = knurl(1.9, 0.7, MAT.black); wheel.rotation.z = Math.PI / 2; wheel.position.set(-12.3, 1.9, 11); base.add(wheel);
  base.add(mesh(box(1.4, 0.9, 2.2, 0.15), MAT.black, { x: -12.7, y: 2.4, z: -6 }));        // power rocker, left rear
  ...
  for (const [x, z] of [[-10.5, 14], [10.5, 14], [-10.5, -15], [10.5, -15]]) base.add(mesh(cyl(1, 1, 0.4), MAT.rubber, { x, y: -0.1, z }));
  tagChildren(base);
  root.add(base);
  parts.base = base;
  ...
  illum.add(mesh(cyl(2.7, 2.7, 0.6), MAT.black, { y: 4.3, z: AXIS_Z }));                   // field diaphragm ring
  const fdRing = knurl(2.85, 0.35, MAT.black); fdRing.position.set(0, 4.15, AXIS_Z); illum.add(fdRing);
  illum.add(mesh(cyl(2.0, 2.0, 0.2), MAT.lamp, { y: 4.65, z: AXIS_Z }));                  // glowing field lens
  illum.add(mesh(box(8, 5.5, 5, 0.5), MAT.creamDark, { y: 4.2, z: -19 }));                 // lamp housing behind the base
  illum.add(mesh(cyl(1.4, 1.4, 1.0), MAT.black, { y: 4.2, z: -21.9, rx: Math.PI / 2 }));   // lamp cap
  const glow = new THREE.PointLight(0xffd77a, 6, 12, 2);
  glow.position.set(0, 5.5, AXIS_Z);
  illum.add(glow);
  tagChildren(illum);
  root.add(illum);
  ...
  arm.add(mesh(box(11, 30, 10, 0.9), MAT.cream, { y: 20, z: PILLAR_FRONT - 5 }));           // pillar, z -16..-6
  arm.add(mesh(box(11, ARM_TOP - ARM_BOTTOM, 21.5, 0.9), MAT.cream, { y: (ARM_TOP + ARM_BOTTOM) / 2, z: -5.2 })); // overhang z -15.95..5.55
  ...
  arm.add(mesh(box(0.15, 2.2, 3.4, 0.05), MAT.black, { x: 5.6, y: 30, z: -9 }));
  arm.add(mesh(box(0.1, 1.2, 2.6, 0.05), MAT.chrome, { x: 5.7, y: 30.2, z: -9 }));
  ...
  stage.add(mesh(box(5.5, 2.6, 3.2, 0.3), MAT.black, { y: STAGE_Y - 1.6, z: PILLAR_FRONT + 2.2 }));
  tagChildren(stage);
  root.add(stage);
  parts.stage = stage;
  ...
  cond.add(mesh(box(2.6, 2.4, 4.5, 0.2), MAT.black, { y: cy - 1.2, z: PILLAR_FRONT + 3.6 }));   // carrier bracket
  ...
  pol.add(mesh(cyl(2.4, 2.4, 0.35), MAT.blackMatte, { y: 4.95, z: AXIS_Z }));
  pol.add(mesh(cyl(1.8, 1.8, 0.1), new THREE.MeshStandardMaterial({ color: 0x4d5a6b, roughness: 0.2, metalness: 0.4, transparent: true, opacity: 0.85 }), { y: 5.17, z: AXIS_Z }));
  pol.add(mesh(box(1.6, 0.3, 0.6, 0.1), MAT.black, { x: 2.8, y: 4.95, z: AXIS_Z })); // rotation tab
